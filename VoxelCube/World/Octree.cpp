...
void Octree::RenderNode(uint32_t LocCode, size_t detail) {
	size_t depth = GetLocDepth(LocCode);
	// If we're at the required level of detail, render
	if (detail == 0) {
		RenderNode(LocCode);
		return;
	}
	...
	for (int i = 0; i < 8; i++) {
		...
		Octree::RenderNode(node->Children[i]->LocCode, detail - 1);
	}
}

void Octree::RenderNode(uint32_t LocCode) {
	size_t depth = GetLocDepth(LocCode);
